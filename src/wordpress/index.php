...
$slug = get_post_meta(get_queried_object_id(), '_sc_slug', true);
...
if (!is_404() && have_posts()) {
    ...
    if ($slug === 'index') { $html = str_replace('<h1>Home</h1>', '<h1 class="hero__title">Welcome to<br>St. Columba Parish</h1>', $html); }
} else {
    foreach (sc_page_data() as $page) { if ($page['slug'] === '404') { $content = $page['body']; } }
}
$html = str_replace('<!--PARISH_BODY-->', $content, $html);
$html = sc_urls($html);
$html = preg_replace('~<script src="[^"]*search-index\.js[^"]*"></script>~', sc_search_script(), $html);
