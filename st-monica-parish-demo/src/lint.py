...
for f in sorted(CONTENT.glob("*.html")):
    ...
    for m in re.finditer(r'href="([^"#]+)(#[^"]*)?"', body):
        ...
        if u.startswith("http"):
            if not any(u.startswith(a.rstrip("/")) for a in allowed_external) and "stmonicasto.archtoronto.org" not in u and "stcolumbatoronto" not in u and "youtube" not in u and "maps.google" not in u and "community.archtoronto" not in u:
    ...
    if f.stem not in ("index", "404", "credits", "privacy") and words < 10: warn(f.name, f"thin page ({words} words)")
