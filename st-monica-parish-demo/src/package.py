...
def main():
    ...
    zip_tree(WEB,OUT/'st-monica-website-netlify-cloudflare.zip')
    zip_tree(THEME,OUT/'st-monica-wordpress-theme.zip','st-monica/')
    (OUT/'START-HERE.txt').write_text('''ST. MONICA'S PARISH DEMO — UPLOAD PACKAGES

NETLIFY: Drag the website folder into Netlify Drop.
CLOUDFLARE: In Workers & Pages, create a Pages project with Direct Upload.
Upload st-monica-website-netlify-cloudflare.zip (or the website folder).
...
Choose st-monica-wordpress-theme.zip, install it, and activate it.
...
address uses St. Monica's public parish email and phone details. Confirm the
production workflow before using the site for real enquiries.
