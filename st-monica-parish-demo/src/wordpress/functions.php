...
function sm_url($path) {
    ...
    $ids = get_option('sm_page_ids', array());
...
function sm_urls($html) {
    ...
    return preg_replace_callback('~(?<=["\x27(\s])/(?!/|wp:)[^"\x27\s<>),]+~u', function ($match) {
        return esc_url(sm_url(html_entity_decode($match[0], ENT_QUOTES, 'UTF-8')));
    }, $html);
}

function sm_setup_pages() {
    if (get_option('sm_installed_version')) { return; }
    $ids = array();
    $created = array();
    foreach (sm_page_data() as $page) {
        ...
        $id = wp_insert_post(array('post_type' => 'page', 'post_status' => 'publish',
            ...
            'meta_input' => array('_sm_slug' => $page['slug'])), true);
        if (!is_wp_error($id)) { $ids[$page['slug']] = $id; $created[$page['slug']] = $page; }
    }
    update_option('sm_page_ids', $ids);
    foreach ($created as $slug => $page) {
        wp_update_post(array('ID' => $ids[$slug], 'post_content' => sm_urls($page['blocks'])));
    }
    ...
    update_option('sm_installed_version', '1.0.0');
    flush_rewrite_rules();
}
add_action('after_switch_theme', 'sm_setup_pages');
...
function sm_search_script() {
    $rows = array();
    foreach (get_option('sm_page_ids', array()) as $slug => $id) {
