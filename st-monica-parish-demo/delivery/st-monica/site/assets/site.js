...
(function () {
  ...
  var siteRoot = runningScript ? new URL('../', runningScript.src) : new URL('/', window.location.href);
  function pageUrl(value) {
    if (!value || value.charAt(0) !== '/' || value.indexOf('//') === 0) return value;
    var match = value.match(/^\/([^?#]*)([?#].*)?$/);
    var path = match ? match[1] : value.slice(1);
    var suffix = match && match[2] ? match[2] : '';
    if (siteRoot.protocol === 'file:' && (!path || path.slice(-1) === '/')) path += 'index.html';
    return new URL(path + suffix, siteRoot).href;
  }
  ...
  var sIndex = window.PARISH_SEARCH || null;
  ...
  function runSearch() {
    ...
    sResults.innerHTML = scored.map(function (x) {
      return '<li><a href="' + pageUrl(x.p.u) + '"><strong>' + x.p.t + '</strong>' + (x.p.s ? ' <span class="search__section">· ' + x.p.s + '</span>' : '') + (x.p.d ? '<span class="search__desc">' + x.p.d + '</span>' : '') + '</a></li>';
    }).join('');
  }
  if (sToggle && sPanel) {
  ...
  function draw() {
    ...
    for (var day = 1; day <= days; day++) {
      ...
      evs.forEach(function (ev) {
        ...
        html += '<a class="month-cal__event month-cal__event--' + (ev.cls || 'parish') + '" href="' + pageUrl(ev.href || '#') + '" title="' + (ev.title + (ev.time ? ', ' + ev.time : '')).replace(/"/g, '&quot;') + '">' + label + '</a>';
    ...
    if (list) {
      list.innerHTML = upcoming.length ? upcoming.map(function (u) {
        return '<li><span class="month-cal__list-date">' + DAYS[u.d.getDay()] + ' ' + u.d.getDate() + '</span> <a href="' + pageUrl(u.ev.href || '#') + '">' + u.ev.title + '</a>' + (u.ev.time ? '<span class="month-cal__list-time">' + u.ev.time + '</span>' : '') + '</li>';
...
/* Portable forms prepare an email without depending on a hosting provider. */
(function () {
  document.querySelectorAll('[data-email-form]').forEach(function (form) {
    form.addEventListener('submit', function (event) {
      ...
      new FormData(form).forEach(function (value, name) {
        ...
        fields.push(name.replace(/[_-]/g, ' ') + ': ' + value);
      });
      var subject = 'Parish enquiry: ' + (form.name || 'contact').replace(/-/g, ' ');
      var address = form.getAttribute('action').split('?')[0];
      window.location.href = address + '?subject=' + encodeURIComponent(subject) + '&body=' + encodeURIComponent(fields.join('\n'));
    });
  });
})();
